-- Make every negative admin check explicitly false-safe.
...
set search_path = ''
as $$
  select coalesce(private.admin_role(check_user_id) in ('owner', 'admin'), false);
$$;
...
begin
  ...
  select
    ...
    coalesce(private.is_admin(caller), false),
